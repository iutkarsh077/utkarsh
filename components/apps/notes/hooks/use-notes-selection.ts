...
import { getStaticNoteBySlug } from "@/data/notes/public-notes";

function resolveFullNote(slug: string): NoteType | null {
...
export function useNotesSelection({
  ...
  useEffect(() => {
    ...
    async function syncSelectedNote() {
      ...
      const fullNote = resolveFullNote(targetSlug);
      ...
      if (fullNote) {
        ...
        setSelectedNote(withDisplayCreatedAt(fullNote));
        if (!isMobile) {
      ...
      if (fallbackSlug && fallbackSlug !== targetSlug) {
        const fallbackFullNote = resolveFullNote(fallbackSlug);
        ...
        if (fallbackFullNote) {
          ...
          setSelectedNote(withDisplayCreatedAt(fallbackFullNote));
  ...
  const handleNoteSelect = useCallback(async (note: NoteType) => {
    ...
    const fullNote = resolveFullNote(note.slug);

    if (fullNote) {
      setSelectedNote((current) => (
        current?.slug === note.slug
          ? withDisplayCreatedAt(fullNote)
          : current
      ));
    }
  }, [persistDesktopSelection]);
